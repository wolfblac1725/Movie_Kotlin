...
fun PopularMoviesScreen(
    ...
    if (movieListState.isLoading) {
        ...
        LazyVerticalGrid(
            ...
            items(movieListState.moviesPopular.size) { index ->
                MovieItem(
                    movie = movieListState.moviesPopular[index],
                    navHostController = navController
                )
                Spacer(modifier = Modifier.height(16.dp))
                if (index >= movieListState.moviesPopular.size - 1 && !movieListState.isLoading) {
